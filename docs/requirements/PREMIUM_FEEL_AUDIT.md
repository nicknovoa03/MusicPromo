...
## v1 App Store — Music Promo workflow only

Scope matches [App Store v1 launch scope](../release/app-store-v1-launch-scope.md): polish what users hit in the core loop, defer SPK/Flyer and heavy new native deps until after review unless you schedule a dedicated regression week.

### In scope (screens)

| Screen | Premium gaps to close |
|--------|------------------------|
| Home | Skeleton grid (not spinner), card/FAB press scale |
| Create picker | Media cards + Add/Cancel press scale |
| Editor | Export, controls, trim toggles press scale |
| Rendering | Close/retry press scale; progress copy already OK |
| Share | Share/Done press scale (partially done) |
| Profile | Light touch only (secondary to create loop) |

### Out of scope for v1 polish pass

- SPK keyboard-controller work (gated off)
- Flyer empty states (gated off)
- `pressto` / Reanimated install (v1.1 — needs full native rebuild QA)
- `react-native-ease` migration (v1.1)

### v1 implementation phases

| Phase | Work | Status |
|-------|------|--------|
| **A** | Home `ProjectGridSkeleton` + `pressFeedback` on Home FAB/cards/header | Done |
| **A** | `pressScaleStyle` on picker → editor → rendering → share CTAs | Done |
| **B** | Profile edit keyboard (optional `keyboard-controller`) | Backlog |
| **C** | `pressto` + Reanimated global press springs | v1.1 |
| **C** | SPK/Flyer items when `LAUNCH_SCOPE=full` | v1.2+ |

Shared helpers: `src/lib/pressFeedback.ts`, `src/components/ProjectGridSkeleton.tsx`, `src/components/Shimmer.tsx`.
...
| `react-native-gesture-handler` | Pan gesture for swipe-to-dismiss | Already installed |
